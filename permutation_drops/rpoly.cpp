...
cout << "\n--------------------------------------------------------------------------- \n";
cout << "\nThe output is written to the file permdrop.out.\n";
...
if (toupper(rflag) == 'Y') 
{
	...
	const char *filename = "permdrop.out";
	ofstream ofile;

	Degree = poly_degree - 1; // Degree of Polynomial is 1 less than the total number of results 
    ...
    //Get the polynomial coefficients and put them in the op vector
    ...
    if (Degree <= 0){
        ...
        ofile.precision(DBL_DIG);
        ofile << "The roots follow:\n";
        ofile << "\n";
		printf("The roots follow:\n\n");
        for (i = 0; i < Degree; i++){
            ofile << zeror[i] << " + " << zeroi[i] << "i" << " \n";
			printf("%.15lf + %.15lf i\n", zeror[i], zeroi[i]);
